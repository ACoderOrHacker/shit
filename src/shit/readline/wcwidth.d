...
private immutable Interval[] doublewidth = [
...
static assert(validateSorted(combining), "Combining table not sorted");
static assert(validateSorted(doublewidth), "Doublewidth table not sorted");

private bool validateSorted(immutable Interval[] table) pure nothrow @nogc @safe
{
    if (table.length < 2)
        return true;
    ...
    {
        if (table[i - 1].end >= table[i].start)
        {
            return false;
        }
    }
...
private bool bisearch(uint32_t u, const Interval[] intervals)
{
    ...
    while (min < max)
    {
        ...
        if (u > intervals[mid].end)
        {
            min = mid + 1;
        }
        else if (u < intervals[mid].start)
        {
            max = mid;
        }
        else
...
export int wcwidth(uint32_t u)
{
    ...
    if (bisearch(u, combining))
        return 0;

    if (bisearch(u, doublewidth))
        return 2;
...
class UnknownCharWidthException : Exception
{
    this(string msg)
    {
        super(msg);
