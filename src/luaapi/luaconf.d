...
*/ /* enable goodies for regular Windows */  /* enable support for DLL */  /* broadly, Windows is C89 */
...
*/
enum LUAI_IS32INT = (UINT_MAX >> 30) >= 3;
...
enum LUA_INT_DEFAULT = LUA_INT_LONGLONG;
enum LUA_FLOAT_DEFAULT = LUA_FLOAT_DOUBLE;
...
enum LUA_INT_TYPE = LUA_INT_DEFAULT;
enum LUA_FLOAT_TYPE = LUA_FLOAT_DEFAULT;
...
extern (D) string l_floatatt(T)(auto ref T n)
{
    import std.conv : to;
...
enum LUA_NUMBER_FMT = "%.15g";
enum LUA_NUMBER_FMT_N = "%.17g";
...
{
    return l_sprintf(s, sz, LUA_INTEGER_FMT, LUAI_UACINT());
}
...
enum LUA_MAXUNSIGNED = ULLONG_MAX;
...
extern (D) auto luai_likely(T)(auto ref T x)
{
    return x;
}

extern (D) auto luai_unlikely(T)(auto ref T x)
{
    return x;
}
